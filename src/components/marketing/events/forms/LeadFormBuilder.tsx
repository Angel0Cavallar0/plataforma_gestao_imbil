    ...
    <div className="max-w-3xl space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Obrigatório — nenhum formulário vive para sempre. Sugestão: último dia do
                evento + 1.
              </p>
            </div>
            <div>
            ...
            <DndContext
              ...
              onDragEnd={(e) => {
                ...
                setCustomFields((fields) => {
                  ...
                  return arrayMove(fields, from, to).map((f, i) => ({ ...f, order: i }));
                });
              }}
            >
              <SortableContext
                ...
                <div className="space-y-2">
                  {customFields.map((field, i) => (
                    <FieldEditor
                      ...
                      onRemove={() =>
                        setCustomFields((f) =>
                          f.filter((_, j) => j !== i).map((x, j) => ({ ...x, order: j })),
                        )
                      }
                    />
                  ))}
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <section>
            <h2 className="mb-2 text-sm font-semibold">Preview (renderização real)</h2>
            <div className="pointer-events-none max-h-[32rem] overflow-y-auto rounded-lg border">
              <PublicLeadForm form={previewData} slug="preview" token="preview" />
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Checkbox de consentimento: “{CONSENT_TEXT}”
            </p>
          </section>
        </div>
      )}
