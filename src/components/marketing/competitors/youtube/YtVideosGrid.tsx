...
/** Grid paginado de vídeos recentes do YouTube (Seção 7). */
export function YtVideosGrid({
  ...
  const visible = videos.slice((current - 1) * PAGE_SIZE, current * PAGE_SIZE);
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {visible.map((v) => (
          <Card key={v.id} className="flex flex-col overflow-hidden">
            {/* Thumbnail servida via proxy CSP-safe (img-src 'self'). */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={`/api/marketing/youtube-thumb/${encodeURIComponent(v.video_id)}`}
